...
void CUberData::LoadAnimations()
{
	...
	const char* names1[] = {
		"UI_NoWeapon_Idle",
		"UI_NoWeapon_Idle", //"Idle_Hands_01",
		"Stand_Aim_Hands_01",
		"Idle_Hands_01",
		"Stand_Aim_Hands_01",
...
void CUberAnim::UnarmedCombat(bool Network, int PlayerState)   // Unarmed Combat with FPS and not FPS
{
	...
	/*r3dAnimation::r3dAnimInfo* animInfo = anim.GetTrack(HandsCombatID);
	if(animInfo && animInfo->GetAnim())
	{
		if(animInfo->GetAnim()->iAnimId == ID)
		{
			return;
		}
	}*/
   ...
   /*if(PlayerState == PLAYER_IDLE)
	HandsCombatID = anim.StartAnimation(ID, ANIMFLAG_RemoveOtherFade | ANIMFLAG_PauseOnEnd, 0.0f, 1.0f, 0.0f);
   else*/
    ...
    SwitchToState(PlayerState, AnimMoveDir);
		//anim.AnimTracks[0].fSpeed = 1.0f * 1.0f;
}
