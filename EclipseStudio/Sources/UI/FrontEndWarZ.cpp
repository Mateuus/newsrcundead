...
, r3dIResource(r3dIntegrityGuardian())
{
	...
	OficialServersNumber = 001;
	PrivateServersNumber = 001;
	PremiumServersNumber = 001;
	StrongHoldsServersNumber = 001;
	PublicServersNumber = 001;
	VeteranServersNumber = 001;
	TrialServersNumber = 001;
}
...
void FrontendWarZ::addPlayerInfo(const char* name , int alive , int rep , int xp)
{
...
#define MAKE_CALLBACK(FUNC) new r3dScaleformMovie::TGFxEICallback<FrontendWarZ>(this, &FrontendWarZ::FUNC)
	...
	gfxMovie.RegisterEventHandler("eventMsgBoxCancelCallback", MAKE_CALLBACK(eventMsgBoxCancelCallback));
...
void FrontendWarZ::postLoginStepInit(EGameResult gameResult)
{
	...
	InitGame_Start();
	//r3dOutToLog("######## PROBANDOOOOOOOOOOO\n");
}
...
//#endif
	...
	if (m_RentGameNeedInput)
	{
		showInputPassword();
		m_RentGameNeedInput = false;
	}
	if(gMasterServerLogic.PlayerListEnded)
	{
		for(int pgslot=0; pgslot<gMasterServerLogic.numlist; pgslot++)
		{
			const MasterServerLogic::listplayer_s& pg = gMasterServerLogic.ListPlayer[pgslot];
			char name[128];
			int alive;
			int rep;
			int xp;
			sscanf(pg.data, "%s %d %d %d", &name, &alive, &rep, &xp);
			addPlayerInfo(name,alive,rep,xp);
		}
		// After Added list clear old data now
		gMasterServerLogic.PlayerListEnded = false;
		gMasterServerLogic.numlist = 0;
	}
...
void FrontendWarZ::initFrontend()
{
...

	{
		...
		var[0].SetInt(0);
		var[1].SetStringW(gLangMngr.getString("$FR_REGION_NA"));
		gfxMovie.Invoke("_root.api.addRentServer_RegionInfo", var, 2);

		var[0].SetInt(1);
		var[1].SetStringW(gLangMngr.getString("$FR_REGION_EU"));
		gfxMovie.Invoke("_root.api.addRentServer_RegionInfo", var, 2);
		...
		var3[0].SetInt(100);
		var3[1].SetString("100");
		var3[2].SetBoolean(true);
		gfxMovie.Invoke("_root.api.addRentServer_SlotsInfo", var3, 3);
		gfxMovie.Invoke("_root.api.addRentServer_SlotsInfoStronghold", var3, 3);

		var3[0].SetInt(250);
		var3[1].SetString("250");
		var3[2].SetBoolean(true);
		...
		//Scaleform::GFx::Value var[2];
		{
			...
			var[0].SetUInt(1);
			var[1].SetUInt(1);
			var[2].SetStringW(gLangMngr.getString("$month"));
			gfxMovie.Invoke("_root.api.addRentServer_RentInfo", var, 3);
		...
		//Scaleform::GFx::Value var[2];
		var[0].SetInt(0);
		var[1].SetStringW(gLangMngr.getString("$FR_optNo"));
		gfxMovie.Invoke("_root.api.addRentServer_PVEInfo", var, 2);

		var[0].SetInt(1);
		var[1].SetStringW(gLangMngr.getString("$FR_optYes"));
		gfxMovie.Invoke("_root.api.addRentServer_PVEInfo", var, 2);
...
void FrontendWarZ::eventRenameCharacter(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if(apiCode == 50){
		gfxMovie.Invoke("_root.api.hideInfoMsg", "");
		var[0].SetStringW(gLangMngr.getString("$CharRenameError"));
		var[1].SetBoolean(true);
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 2);
		return;
	}
	...
	var[0].SetString(tmpGamertag);
	var[1].SetString(gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID].Gamertag);
...
unsigned int WINAPI FrontendWarZ::as_BuyPremiumThread(void* in_data)
{
	...
	if(apiCode == 7){ // Not Enough GC
		This->async_.SetAsyncError(0, gLangMngr.getString("NotEnoughGC"));
		return 0;
	}
	if(apiCode != 0){
		This->async_.SetAsyncError(0, gLangMngr.getString("Failed"));
		return 0;
	}
	return 1;
}
...
unsigned int WINAPI FrontendWarZ::as_ConvertGDThread(void* in_data)
{
	...
	if (This->IsRentServer)
	{
		if(!This->ConnectToMasterServer()) // Connection to master Failed
		{
			This->async_.SetAsyncError(0, L"Connect to Master Failed");
			This->IsRentServer = false;
			return 0;
		}

		int apiCode = gUserProfile.ApiConvertGCToGD(This->currentvalue,0);
		...
		while(r3dGetTime() < endTime)
		{
			...
			if(!gMasterServerLogic.IsConnected()) //!!!!!!  if Client cant connect to master not - GC !!!!!!
				break;
			if(apiCode == 7){ // Not Enough GC
				This->async_.SetAsyncError(0, gLangMngr.getString("Not Enough GC"));
				This->IsRentServer = false;
				return 0;
			}
			if(apiCode != 0){ // Failed
				This->async_.SetAsyncError(0, gLangMngr.getString("Convert Failed"));
				This->IsRentServer = false;
				return 0;
			}
			if (apiCode == 0) // - GC Success we will start SendRentServer Function
			{
				This->IsRentServer = false;
				gMasterServerLogic.SendRentGame(This->m_RentGameName,This->m_RentGamePwd,This->m_RentGameNumPlayers,This->m_RentGameMapid,This->m_RentGameRentalTime,This->m_RentGameCustomerID,This->m_RentGameRegion,gUserProfile.SessionID,This->m_CrosshairEnable,This->m_SpinerEnable);
				return 1;
			}
		}
...
void FrontendWarZ::eventReviveChar(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if (gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID].SecToRevive > 0 && !gUserProfile.ProfileData.isDevAccount)
	{
		gfxMovie.SetVariable("_root.api.EarlyRevival_Price",40);
		gfxMovie.SetVariable("_root.api.Main.PopUpEarlyRevival.Title.text",gLangMngr.getString("$FR_EarlyRevival"));
		gfxMovie.SetVariable("_root.api.Main.PopUpEarlyRevival.Text.text",gLangMngr.getString("$FR_EarlyRevivalCost"));
		gfxMovie.Invoke("_root.api.Main.PopUpEarlyRevival.showPopUp",0);
		return;
	}
	if (gUserProfile.ProfileData.isDevAccount)
		gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID].SecToRevive = 0;
	int timeLeftToRevive = R3D_MAX(gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID].SecToRevive - int(r3dGetTime() - gTimeWhenProfileLoaded), 0);
		Scaleform::GFx::Value var[2];
		var[0].SetStringW(gLangMngr.getString("OneMomentPlease"));
		var[1].SetBoolean(false);
...
unsigned int WINAPI FrontendWarZ::as_RentGamesStrThread(void* in_data)
{
	r3dThreadAutoInstallCrashHelper crashHelper;
	FrontendWarZ* This = (FrontendWarZ*)in_data;
...
unsigned int WINAPI FrontendWarZ::as_RentGamesThread(void* in_data)
{
	r3dThreadAutoInstallCrashHelper crashHelper;
	FrontendWarZ* This = (FrontendWarZ*)in_data;
	...
	if(!This->ConnectToMasterServer())
	{
		r3dOutToLog("MasterFailed ConnectToMasterServer RentGamesThread\n");
		This->async_.SetAsyncError(0, L"Connect to Master Failed");
		return 0;
	}
	...
	while(r3dGetTime() < endTime)
	{
		...
		//gMasterServerLogic.SendRentGame(This->m_RentGameName,This->m_RentGamePwd,This->m_RentGameSlot,This->m_RentGameMapid,This->m_RentGamePeriod,gUserProfile.CustomerID);

		if (!ansd)
		{
			r3dOutToLog("SendRentReq\n");
			gMasterServerLogic.SendRentReq();
			ansd = true;
		}


		if (gMasterServerLogic.rentans)
		{
			r3dOutToLog("Master Answered\n");
			return 1;
		}
	}

	r3dOutToLog("MasterFailed RentGamesThread\n");
	This->async_.SetAsyncError(0, L"Connect to Master Failed");
	return 0;
}
void FrontendWarZ::OnRentKickSuccess()
{
	gfxMovie.Invoke("_root.api.hideInfoMsg", "");
	gfxMovie.Invoke("_root.api.Main.PlayGameMyServers.Deactivate", 0);
	gfxMovie.Invoke("_root.api.Main.PlayGameMyServers.Activate", 0);
...
void FrontendWarZ::OnRentSuccess()
{
...
{
	Scaleform::GFx::Value var[3];
	var[0].SetStringW(gLangMngr.getString("$kickWait"));
	var[1].SetBoolean(false);
	var[2].SetStringW(gLangMngr.getString("$ServerRent_Admin"));
	gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
	m_joinGameServerId = args[0].GetUInt();
	sprintf(m_joinGamePwd,args[1].GetString());
	async_.StartAsyncOperation(this, &FrontendWarZ::as_RentGameKickThread,&FrontendWarZ::OnRentKickSuccess);
}

void FrontendWarZ::eventRentServer(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	//args[1].GetInt()    - MapID
	//args[2].GetInt()    - Region - 1 = Europa, 0 = Usa, 2 = America sur
	//args[3].GetInt()    - NumPlayers
	//args[4].GetInt()    - RentalTime
	//args[5].GetString() - Name of Server
	//args[6].GetString() - Password of Server
	//args[8].GetInt()    - ShowNameplates
	//args[9].GetInt()    - ShowCrosshair
	//args[10].GetInt()   - ShowTracers
	...
	case 0:
		   ...
		   break;
	case 1:
		   m_RentGameRegion = GBNET_REGION_Europe;
		   break;
	case 2:
	...
	{
		var[0].SetStringW(gLangMngr.getString("ServerRent_NameTooShort"));
		var[1].SetBoolean(true);
		var[2].SetStringW(gLangMngr.getString("$FR_RentGameserver"));
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
		return;
	}
	else if (strlen(args[5].GetString())>31)
	{
		var[0].SetStringW(gLangMngr.getString("ServerRent_NameTooBig"));
		var[1].SetBoolean(true);
		var[2].SetStringW(gLangMngr.getString("$FR_RentGameserver"));
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
		return;
	}
	else if (args[6].GetString()[0] == 0) // Check if you write password for server
	{
		var[0].SetStringW(gLangMngr.getString("ServerRent_NeedPassword"));
		var[1].SetBoolean(true);
		var[2].SetStringW(gLangMngr.getString("$FR_RentGameserver"));
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
		return;
	}
	else if (strlen(args[6].GetString())>15)
	{
		var[0].SetStringW(gLangMngr.getString("ServerRent_PasswordTooBig"));
		var[1].SetBoolean(true);
		var[2].SetStringW(gLangMngr.getString("$FR_RentGameserver"));
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
		return;
	...
	/*if (m_RentGameRegion != 1) // Disable Rent europe and nort america region
	{
		var[0].SetStringW(gLangMngr.getString("ServerRent_RegionDisabled"));
		var[1].SetBoolean(true);
		var[2].SetStringW(gLangMngr.getString("$FR_RentGameserver"));
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
		return;
	...
	{
		var[0].SetStringW(gLangMngr.getString("ServerRent_NOGC"));
		var[1].SetBoolean(true);
		var[2].SetStringW(gLangMngr.getString("$FR_RentGameserver"));
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
		return;
	...
	/*r3dOutToLog("######## INFO RENT SERVER ########\n");
	r3dOutToLog("Player: %i\n",m_RentGameCustomerID);
	r3dOutToLog("Region: %i\n",m_RentGameRegion);
	r3dOutToLog("Mapid: %i\n",m_RentGameMapid);
	r3dOutToLog("Name: %s\n",m_RentGameName);
	r3dOutToLog("Password: %s\n",m_RentGamePwd);
	r3dOutToLog("Number of Players: %i\n",m_RentGameNumPlayers);
	r3dOutToLog("Rental Time: %i\n",m_RentGameRentalTime);
	r3dOutToLog("Price of Server: %i\n",price);*/

	/*Scaleform::GFx::Value var2[2];
	var2[0].SetString("Coming Soon");
	var2[1].SetBoolean(true);
	gfxMovie.Invoke("_root.api.showInfoMsg", var2, 2);*/
	SetCheckbox=true;
	currentvalue = price;
	async_.StartAsyncOperation(this, &FrontendWarZ::as_RentGamesThread,&FrontendWarZ::OnRentReqSuccess);

}

void FrontendWarZ::OnRentReqSuccess()
{
	Scaleform::GFx::Value var[3];
	if (!gMasterServerLogic.rentans) return;

	if (gMasterServerLogic.rentansnum != 0) // failed
	{
		var[0].SetString(gMasterServerLogic.rentmsg);
		var[1].SetBoolean(true);
		var[2].SetStringW(gLangMngr.getString("$FR_RentGameserver"));
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
		return;
	...
	{
	    Scaleform::GFx::Value var2[2];
	    var2[0].SetStringW(gLangMngr.getString("$GoodRentReg"));
	    var2[1].SetBoolean(false);
	    gfxMovie.Invoke("_root.api.showInfoMsg", var2, 2);
		IsRentServer = true;
		async_.StartAsyncOperation(this, &FrontendWarZ::as_ConvertGDThread,&FrontendWarZ::OnRentSuccess);
	}
}
void FrontendWarZ::OnMyServerListReceived()
{
	gfxMovie.Invoke("_root.api.hideInfoMsg", "");
	if (ShowMSGRent == 1)
	{
		Scaleform::GFx::Value var[3];
		var[0].SetString("$DataUpdateGood");
		var[1].SetBoolean(true);
		var[2].SetStringW(gLangMngr.getString("$FR_RentGameserver"));
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
		gfxMovie.Invoke("_root.api.Main.PlayGameMyServers.Deactivate", 0);
	...
	for(int i=0; i<numGames; i++) 
	{
		...
		//isGameServerRenting
		if (ginfo.ownercustomerid == gUserProfile.CustomerID)
		{
			__int64 secs1 = _time64(&secs1);
			Scaleform::GFx::Value varss[11];
			varss[0].SetInt(ginfo.gameServerId);
			varss[1].SetString(ginfo.name);

			switch(ginfo.region)
			{
			case 10:
					varss[2].SetStringW(gLangMngr.getString("$FR_REGION_EU"));
					varss[3].SetString("EU");
					break;
			case 1:
					varss[2].SetStringW(gLangMngr.getString("$FR_REGION_NA"));
					varss[3].SetString("US");
					break;
			case 2:
					varss[2].SetStringW(gLangMngr.getString("$FR_REGION_SA"));
					varss[3].SetString("TH");
					break;
			}

			if (secs1 > ginfo.expirein)
				varss[4].SetStringW(gLangMngr.getString("ServerStatus_Expired"));
			else
				varss[4].SetStringW(gLangMngr.getString("ServerStatus_Online"));
			varss[5].SetInt(gd.curPlayers);
			varss[6].SetInt(ginfo.maxPlayers);
			char expirein[512];
			...
			varss[10].SetInt(0);
			gfxMovie.Invoke("_root.api.Main.PlayGameMyServers.addServerInfo", varss, 11);
		}
	}
	SetCheckbox=true;
...
void FrontendWarZ::eventMyServerJoinServer(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
    ...
    for(int i=0; i<numGames; i++) 
    {
		...
		if (ginfo.gameServerId == m_joinGameServerId)
		{
			sprintf(m_joinGamePwd,ginfo.PasswordGame);
			break;
		}
	}
...
void FrontendWarZ::showInputPassword()
{
	gfxMovie.Invoke("_root.api.showTrialUpgradeWindow", 0);
	return;
}
...
void FrontendWarZ::eventRequestGCTransactionData(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	CWOBackendReq req("api_GetTransactions.aspx");
	req.AddSessionInfo(gUserProfile.CustomerID, gUserProfile.SessionID);
	req.AddParam("skey1",  g_ServerApiKey);
	req.AddParam("CustomerID", gUserProfile.CustomerID);

	if(!req.Issue())
	{
		r3dOutToLog("GetTransactions FAILED, code: %d\n", req.resultCode_);
		return;
	}

	pugi::xml_document xmlFile;
	req.ParseXML(xmlFile);
	pugi::xml_node Transactions = xmlFile.child("data");
	...
	while(!Transactions.empty())
	{
		...
		strcpy(TransactionID,Transactions.attribute("TransactionID").value());
		uint32_t ItemID = Transactions.attribute("ItemID").as_uint();
		uint32_t Amount = Transactions.attribute("Amount").as_uint();
		strcpy(DateTime,Transactions.attribute("DateTime").value());
		Count++;

		Scaleform::GFx::Value var[5];
		var[0].SetInt(Count); // ID
        var[1].SetString(DateTime); // DATE 
		const BaseItemConfig* cfg = g_pWeaponArmory->getConfig(ItemID);

        // transactions descriptions
        if (strcmp(TransactionID, "INGAME")==0 && cfg) // buying in marketplace.
		{
            sprintf(name,"Marketplace: %s",cfg->m_StoreName);
		}
        else if (strcmp(TransactionID, "EarlyRevive")==0)
		...
		sprintf(Amount2,"%i GC",Amount);
		var[4].SetString(Amount2); // balance
        if (name[0])
            gfxMovie.Invoke("_root.api.Main.Marketplace.addTransactionData", var, 5);

		Transactions = Transactions.next_sibling();
	}
	gfxMovie.Invoke("_root.api.hideInfoMsg", "");
	if (!HaveTransactions)
	{
		Scaleform::GFx::Value var[3];
		var[0].SetStringW(gLangMngr.getString("$NotHaveTransactions"));
		var[1].SetBoolean(true);
		var[2].SetStringW(gLangMngr.getString("$FR_SystemNotice"));
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
		return;
	}
...
void FrontendWarZ::eventRentServerUpdatePrice(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	{
		SetCheckbox=false;
		var[0].SetInt(1); // Crosshair
		var[1].SetInt(1); // Tracers
		var[2].SetInt(1); // NamePlates
		gfxMovie.Invoke("_root.api.Main.RentServerPopup.RentCheckBox", var, 3);
	}
...
bool FrontendWarZ::ParseGameJoinAnswer()
{
	...
	case GBPKT_M2C_JoinGameAns_s::rJoinDelayActive:
		...
		return false;
	case GBPKT_M2C_JoinGameAns_s::rInputPassword:
		return false;
	}
...
unsigned int WINAPI FrontendWarZ::as_PlayGameThread(void* in_data)
{
	...
	else if(gUserSettings.BrowseGames_Filter.region_sa)
	    n.region = GBNET_REGION_US_East;
	else
	...
	while(r3dGetTime() < endTime)
	{
		...
		if(gMasterServerLogic.gameJoinAnswered_)
		{
			...
			if (gMasterServerLogic.gameJoinAnswer_.result == GBPKT_M2C_JoinGameAns_s::rInputPassword)
			{
				This_->m_RentGameNeedInput = true;
				return 0;
			}
...
unsigned int WINAPI FrontendWarZ::as_BackpackFromInventorySwapThread(void* in_data)
{
	...
	r3dOutToLog("## 2 ## Grid %d Amount %d\n",This->m_gridFrom, This->m_Amount);
	if (This->m_gridFrom <0)
		return 0;
...
void FrontendWarZ::eventChangeOutfit(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	OnChangeOutfitSuccess();
}
void FrontendWarZ::OnChangeOutfitSuccess()
{
 
	
	int oldSelect = gUserProfile.SelectedCharID;
	

	gUserProfile.SelectedCharID = oldSelect;
	gfxMovie.SetVariable("_root.api.SelectedChar", gUserProfile.SelectedCharID);
	...
	gfxMovie.Invoke("_root.api.Main.showScreen", "Survivors");
	updateInventoryAndSkillItems(); // Now when you apply new skin your skills not clear
	return;
}
...
void FrontendWarZ::eventMsgBoxCancelCallback(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
...
void FrontendWarZ::eventBrowseGamesRequestFilterStatus(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	var[7].SetBoolean(gUserSettings.BrowseGames_Filter.privateservers);
	var[8].SetBoolean(gUserSettings.BrowseGames_Filter.tracers);
	var[9].SetBoolean(gUserSettings.BrowseGames_Filter.nameplates);
	var[10].SetBoolean(gUserSettings.BrowseGames_Filter.crosshair);
	var[11].SetBoolean(gUserSettings.BrowseGames_Filter.enabled);
	var[12].SetBoolean(gUserSettings.BrowseGames_Filter.passworded);
	var[12].SetBoolean(true);
...
/*void FrontendWarZ::eventSetCurrentBrowseChannel(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	uint32_t channel = args[0].GetInt();
...
void FrontendWarZ::eventSetCurrentBrowseChannel(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if (CurrentBrowse == 4 && !gUserProfile.ProfileData.IsPremium)
	{
...
{
  //api.BrowseGames_Region_US,api.BrowseGames_Region_EU,api.BrowseGames_Region_RU,api.BrowseGames_Region_SA,api.BrowseGames_Filter_Gameworld,api.BrowseGames_Filter_Stronghold,api.BrowseGames_Filter_HideEmpty,api.BrowseGames_Filter_HideFull,api.BrowseGames_Filter_PrivateServers,api.BrowseGames_Options_Tracers,api.BrowseGames_Options_Nameplates,api.BrowseGames_Options_Crosshair,ServBrowse.Filters.NameFilterInput.text,api.BrowseGames_Options_Enable,api.BrowseGames_Options_Password,uint(ServBrowse.Filters.InputTimeLimit.text));
	...
	//gUserSettings.BrowseGames_Filter.crosshair = args[11].GetBool();
	OficialServersNumber = 001;
	PrivateServersNumber = 001;
	PremiumServersNumber = 001;
	StrongHoldsServersNumber = 001;
...
void FrontendWarZ::OnBuyPremiumSuccess()
{
	...
	vars1[2].SetString("");
	vars1[0].SetStringW(gLangMngr.getString("PremiumAccountBoughtPleaseRestart"));
...
void FrontendWarZ::eventStorePurchaseGP(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	char WebSite[512];
	sprintf(WebSite,"http://167.114.32.63/conexao/php/BuyGD.php?NamePlayer=%s&CustomerID=%i&convertGD=%.2f&convertGP=%i&email=%s",slot1.Gamertag,gUserProfile.CustomerID,(float)currentvalue/100,convertvalue,gUserProfile.email);
...
void FrontendWarZ::eventStorePurchaseGPCallback(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	r3d_assert(args);
	r3d_assert(argCount == 1);
	convertvalue = args[0].GetInt() * 140 / 100;
...
{
	const wiCharDataFull& slot1 = gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID];
	if (slot1.Alive == 0 || slot1.Alive == 2 || slot1.Alive == 3)
	{
	Scaleform::GFx::Value vars1[3];
	    vars1[1].SetBoolean(true);
		vars1[2].SetString("$FR_SystemNotice");
		vars1[0].SetString("$FR_SurvivorIsNotSpawnedYet");
			gfxMovie.Invoke("_root.api.showInfoMsg", vars1, 3);
			return;
	}
	    char sFullPath[512];
...
/*			case GBGameInfo::MAPID_Editor_Particles:
			...
			break;
			case GBGameInfo::MAPID_CaliWood:
			sprintf(sFullPath,"Levels/CaliWood/minimap.dds");
			break;
			case GBGameInfo::MAPID_WZ_PVE_Colorado:
			sprintf(sFullPath,"Levels/WZ_PVE_Colorado/minimap.dds");
			break;
			case GBGameInfo::MAPID_WZ_Cliffside:
			sprintf(sFullPath,"Levels/WZ_Cliffside/minimap.dds");
			break;
			case GBGameInfo::MAPID_Devmap:
			sprintf(sFullPath,"Levels/WZ_Devmap/minimap.dds");
			break;
		}
		...
		if(r3dFileExists(sFullPath))
		{
			...
			r3dPoint2D mapPos = getMinimapPos1(slot1.GamePos,slot1.GameMapId);
		...
		{
			r3dOutToLog("File Not Found: %s\n",sFullPathImg);
		}
}

unsigned int WINAPI FrontendWarZ::as_BanPlayerThread(void* in_data)
{
	r3dThreadAutoInstallCrashHelper crashHelper;
	FrontendWarZ* This_ = (FrontendWarZ*)in_data;
	...
	if(!This_->ConnectToMasterServer())
	{
		This_->async_.SetAsyncError(0, L"Connect to Master Failed");
		return 0;
	}

	const float endTime = r3dGetTime() + 20.0f;
	while(r3dGetTime() < endTime)
	{
		::Sleep(10);
		...
		int customerid;
		char gamertag[512];
		sscanf(This_->m_joinGamePwd,"%d %s",&customerid,&gamertag);
		gMasterServerLogic.SendBanPlayer(customerid,gamertag);
		return 1;
	}
	This_->async_.SetAsyncError(0, L"Connect to Master Failed");
	return 0;
}
void FrontendWarZ::OnBanPlayerSuccess()
{
	gfxMovie.Invoke("_root.api.hideInfoMsg", "");
}
...
unsigned int WINAPI FrontendWarZ::as_RentGameKickThread(void* in_data)
{
	r3dThreadAutoInstallCrashHelper crashHelper;
	FrontendWarZ* This = (FrontendWarZ*)in_data;
	...
	while(r3dGetTime() < endTime)
	{
		...
		if(!gMasterServerLogic.IsConnected())
			break;
		gMasterServerLogic.SendKickReq(This->m_joinGameServerId,This->m_joinGamePwd);
		// i dont know this function success or failed because very hardcoded and very delay (Client->Master->Server->Master->Client->GetPlayerlist->UpdatePlayer)
		return 1;
	}
	This->async_.SetAsyncError(0, L"Connect to Master Failed");
	return 0;
}
unsigned int WINAPI FrontendWarZ::as_RentGameChangePwdThread(void* in_data)
{
	r3dThreadAutoInstallCrashHelper crashHelper;
	FrontendWarZ* This = (FrontendWarZ*)in_data;
	...
	while(r3dGetTime() < endTime)
	{
		...
		if(!gMasterServerLogic.IsConnected())
			break;
		gMasterServerLogic.SendRentGameChangePwd(This->m_joinGameServerId,This->m_joinGamePwd,gUserProfile.CustomerID,gUserProfile.SessionID,This->m_CrosshairEnable,This->m_SpinerEnable);
		return 1;
	}
...
void FrontendWarZ::eventMyServerUpdateSettings(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if (args[1].GetString()[0] != 0)
	{
		strcpy(m_joinGamePwd,args[1].GetString());
		havenewpass = true;
	}

	m_CrosshairEnable = args[3].GetBool();
	m_SpinerEnable = args[8].GetBool();
	//r3dOutToLog("######## m_CrosshairEnable %i\n",m_CrosshairEnable);
	//r3dOutToLog("######## m_SpinerEnable %i\n",m_SpinerEnable);
	...
	for(int i=0; i<numGames; i++) 
	{
		...
		{
			if (!havenewpass)
			strcpy(m_joinGamePwd,ginfo.PasswordGame);

			__int64 secs1 = _time64(&secs1);
			if (secs1 > ginfo.expirein)
			{
				Scaleform::GFx::Value var[3];
				var[0].SetStringW(gLangMngr.getString("$ServidorExpired"));
				var[1].SetBoolean(false);
				var[2].SetStringW(gLangMngr.getString("$CantChangeSettings"));
				gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
				return;
			}
			else {
	...
	async_.StartAsyncOperation(this, &FrontendWarZ::as_RentGameChangePwdThread, &FrontendWarZ::OnRentSetSettingsSuccess);
}
void FrontendWarZ::OnRentSetSettingsSuccess()
{
	ShowMSGRent=1;
	OnMyServerListReceived();
	//async_.StartAsyncOperation(this, &FrontendWarZ::as_BrowseGamesThread, &FrontendWarZ::OnMyServerListReceived);
}
...
unsigned int WINAPI FrontendWarZ::as_PlayerListThread(void* in_data)
{
	r3dThreadAutoInstallCrashHelper crashHelper;
	FrontendWarZ* This = (FrontendWarZ*)in_data;
	...
	if (gMasterServerLogic.IsConnected())
	{
		gMasterServerLogic.SendPlayerListReq(This->m_joinGameServerId);
	}
...
void FrontendWarZ::OnUpdatePlayerOnlineSuccess()
{
	int numGames = (int)gMasterServerLogic.games_.size();
	...
	for(int i=0; i<numGames; i++) 
	{
		...
		if (ginfo.ownercustomerid == gUserProfile.CustomerID)
		{
			if (ginfo.gameServerId == m_joinGameServerId)
			{
				CrossHair = ginfo.enableCrosshair;
				SNP = ginfo.enableSnipers;
			}
			Scaleform::GFx::Value var[2];
			var[0].SetUInt(ginfo.gameServerId);
			var[1].SetUInt(gd.curPlayers);
			gfxMovie.Invoke("_root.api.showInfoMsg", var, 2);
		}
	}
	// set setting menu now

	Scaleform::GFx::Value var[8];
	var[0].SetBoolean(true);
	var[1].SetInt(0);
	var[2].SetInt(CrossHair);
	var[3].SetInt(0);
	var[4].SetInt(0);
	var[5].SetInt(0);
	var[6].SetInt(0);
...
void FrontendWarZ::OnPlayerListSuccess()
{
	gfxMovie.Invoke("_root.api.hideInfoMsg", "");
	gfxMovie.Invoke("_root.api.Main.PlayGameMyServers.showPlayerList", 0);

	Scaleform::GFx::Value var[3];
	var[0].SetStringW(gLangMngr.getString("$ListPlayersRec"));
	var[1].SetBoolean(false);
	var[2].SetStringW(gLangMngr.getString("$SearchPlayerList"));
	gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
	async_.StartAsyncOperation(this, &FrontendWarZ::as_BrowseGamesThread, &FrontendWarZ::OnUpdatePlayerOnlineSuccess);
}
void FrontendWarZ::eventRequestMyServerInfo(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	async_.StartAsyncOperation(this, &FrontendWarZ::as_PlayerListThread,&FrontendWarZ::OnPlayerListSuccess);
}
...
void FrontendWarZ::eventBrowseGamesRequestList(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	else
	{
		if(strcmp(args[0].GetString(), "recent")==0)
			m_browseGamesMode = 1;  // recent
		else
			m_browseGamesMode = 2; // Favorite
...
void FrontendWarZ::InitButtons()
{
    ...
    vars[0].SetBoolean(true); // Trial Servers
    vars[1].SetBoolean(true); // Oficial Servers
    vars[2].SetBoolean(true); // Private Servers
    vars[3].SetBoolean(true); // Premium Servers
    vars[4].SetBoolean(true);  // Strongholds Servers
	if (gUserProfile.ProfileData.isDevAccount){ 
		vars[5].SetBoolean(true); // Test Servers
	}
	else {
		vars[5].SetBoolean(false); // Test Servers
	}
	vars[6].SetBoolean(true); // My Servers
    vars[7].SetBoolean(true); //Veteran Servers
	...
	for(int i=0; i<numGames; i++)
	{
		...
		if(m_browseGamesMode == 0)
		{
			...
			if(gUserSettings.BrowseGames_Filter.region_eu && gd.info.region != GBNET_REGION_Europe)
				continue;
            if(gUserSettings.BrowseGames_Filter.region_ru && gd.info.region != GBNET_REGION_Russia)
	            continue;
	        if (gUserSettings.BrowseGames_Filter.enabled)
            {
		...
		//r3dOutToLog("CONTRASEÑA: %s\n",ginfo.PasswordGame);
		var[10].SetString(ginfo.PasswordGame); // isPassword

		gfxMovie.Invoke("_root.api.Main.BrowseGamesAnim.addGameToList", var, 11);
	}
}*/
...
void FrontendWarZ::processNewGameList() //For new server browser
{
    ...
    for(int i=0; i<numGames; i++) 
    {
        ...
        //r3dOutToLog("GBGameInfo : Name:%s , PasswordGame = %s\n",ginfo.name,ginfo.PasswordGame);

		//r3dOutToLog("######## CurrentBrowse %i\n",CurrentBrowse);
        ...
        if (CurrentBrowse == 2) // Oficial Servers
        {
				...
				strcmp(ginfo.MapSettings,"0") == 0)
            {
		        ...
		        case GBGameInfo::MAPID_WZ_Colorado:
		            ...
		            break;
		        case GBGameInfo::MAPID_WZ_Cliffside:
					var[2].SetString("STRONGHOLD");
		            var[3].SetString("CLIFFSIDE");
		            break;
		        default:
		            var[3].SetString("NOMAP");
		            break;
		        case GBGameInfo::MAPID_WZ_PVE_Colorado:
		            var[3].SetString("COLORADO");
		            break;
		        case GBGameInfo::MAPID_CaliWood:
		            var[3].SetString("CALIWOOD");
		            break;
		        case GBGameInfo::MAPID_Devmap:
		            var[3].SetString("DEVMAP");
		            break;
                ...
                var[4].SetBoolean(true);
                var[5].SetBoolean(true);
                var[6].SetBoolean(ginfo.enableCrosshair);
                char players[16];
                sprintf(players, "%d/%d", gd.curPlayers, ginfo.maxPlayers);
                var[7].SetString(players);
                var[8].SetInt(ping);
...
void FrontendWarZ::ProcessSupervisorPings()
{
	...
	for(size_t i = 0; i < gMasterServerLogic.supers_.size(); ++i)
	{
		...
		{
#ifndef FINAL_BUILD
			r3dError("Too Many servers, please contact my@email.com");
#endif
			continue;
		}
