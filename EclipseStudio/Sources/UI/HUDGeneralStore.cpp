...
unsigned int WINAPI HUDGeneralStore::as_BuyItemThread(void* in_data)
{
	...
	gClientLogic().SendBuyItemReq(This->ItemID,This->storeBuyPrice_,This->storeBuyPriceGD_);
...
void HUDGeneralStore::eventBuyItem(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if ((gUserProfile.ProfileData.GameDollars-storeBuyPriceGD_)<0)
	{
	...
	if ((gUserProfile.ProfileData.GamePoints-storeBuyPrice_)<0)
	{
		Scaleform::GFx::Value var[2];
		var[0].SetStringW(gLangMngr.getString("InGameUI_ErrorMsg_NoMoney"));
		var[1].SetBoolean(true);
