...
r3dPoint2D getMinimapPos(const r3dPoint3D& pos)
{
	...
	//r3dOutToLog("######## left_corner_x %f bottom_corner_y %f x_size %f y_size %f\n",left_corner_x,bottom_corner_y,x_size,y_size);
...
void	HUDPause::eventMissionAccept(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
 ...
 if (id == 2) // Mission Zombie Kill
 {
	Deactivate();
	PKT_C2S_PlayerAcceptMission_s n1;
	n1.id = 2;
     p2pSendToHost(gClientLogic().localPlayer_, &n1, sizeof(n1), true);
	 hudMain->addMissionInfo("SUPER ZOMBIE HUNTER");
	 //hudMain->addMissionObjective(0,"PICK UP ITEM",false,"0/1",true);
	 hudMain->addMissionObjective(0,"KILL 5 SUPER ZOMBIES",false,"0/5",true);
	 hudMain->showMissionInfo(true);
	 gClientLogic().localPlayer_->CurLoadout.Mission2 = 1;
 }
}
...
void HUDPause::eventShowContextMenuCallback(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	}else if(wc)
	{
		...
		if((wc->category == storecat_MELEE || (wc->category >= storecat_ASR && wc->category <= storecat_GRENADE)))
		{
			...
			if (wc->m_itemID == WeaponConfig::ITEMID_Repairkit)
				return;
...
void HUDPause::eventContextMenu_Action(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	const WeaponAttachmentConfig* clipAttach = g_pWeaponArmory->getAttachmentConfig(wi.itemID);
...
case 7: //combine clips
		if (wac)
		{
			PKT_C2S_StackClip_s n;
			n.slotId = slotID;
			p2pSendToHost(plr,&n,sizeof(n));
		}
		return;
	...
	const WeaponConfig* wc = g_pWeaponArmory->getWeaponConfig(wi.itemID);
		if(wc)
		{
			...
			if(wc->category >= storecat_ASR && wc->category <= storecat_SMG)
			{
					...
					Scaleform::GFx::Value var[2];
					var[0].SetString("Error al descargar");
					var[1].SetInt(2);
					gfxMovie.Invoke("_root.api.Main.Inventory.addContextMenuOption", var, 2);
					return;
			}
...
void HUDPause::showMessage(const wchar_t* text)
{
	
    Scaleform::GFx::Value var[3];
    var[0].SetStringW(text);
...
/*void HUDPause::eventBackpackUseItem(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
...

	{
		...
		if(wi.itemID == 301320 || wi.itemID == 301365 || wi.itemID == 301370 || wi.itemID == 301319 || wi.itemID == 301335 || wi.itemID == 301332
		|| wi.itemID == 301328 || wi.itemID == 301339 || wi.itemID == 301327 || wi.itemID == 301324 || wi.itemID == 301318 || 
		wi.itemID == 301366 || wi.itemID == 301331 || wi.itemID == 301357 || wi.itemID == 301320 || wi.itemID == 301331)
		{
			Deactivate();
			hudCraft->Activate();
			return;
	...
	else if(wi.itemID == WeaponConfig::ITEMID_AirHorn)
	{
		...
		wi.itemID == WeaponConfig::ITEMID_BlockDoorWood ||
        ...
        wi.itemID == WeaponConfig::ITEMID_BlockDoorWood2M01 ||
        wi.itemID == WeaponConfig::ITEMID_BlockWallMetal2M01 ||
        wi.itemID == WeaponConfig::ITEMID_BlockWallBrickTall ||
        wi.itemID == WeaponConfig::ITEMID_BlockWallWood2M01 ||
        wi.itemID == WeaponConfig::ITEMID_BlockWallBrickShort2 ||
        wi.itemID == WeaponConfig::ITEMID_BlockFarm ||
        wi.itemID == WeaponConfig::ITEMID_BlockSolarWater2 ||
        wi.itemID == WeaponConfig::ITEMID_BlockLight2 ||
        wi.itemID == WeaponConfig::ITEMID_BlockFarmCrate ||
		wi.itemID == WeaponConfig::ITEMID_PersonalLocker ||
		wi.itemID == WeaponConfig::ITEMID_BlockEnergy)
	{
		Scaleform::GFx::Value var[3];
	...
	else
	{
		...
		{
			Scaleform::GFx::Value var[3];
			var[0].SetString("$InfoMsg_MaxHealthAlready");
			var[1].SetBoolean(true);
			var[2].SetString("");
			gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
			return;
		...
		{
			Scaleform::GFx::Value var[3];
			var[0].SetString("$InfoMsg_NoTocixInBlood");
			var[1].SetBoolean(true);
			var[2].SetString("");
			gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
			return;
...
void HUDPause::eventBackpackUseItem(int slotID)
{
	...
	else if(wi.itemID == WeaponConfig::ITEMID_Binoculars || wi.itemID == WeaponConfig::ITEMID_RangeFinder || wi.itemID == WeaponConfig::ITEMID_AirHorn)
	{
        ...
        wi.itemID == WeaponConfig::ITEMID_BlockFarmCrate ||
		wi.itemID == WeaponConfig::ITEMID_PersonalLocker ||
		wi.itemID == WeaponConfig::ITEMID_IndustGenerator ||
		wi.itemID == WeaponConfig::ITEMID_ClayMore ||
		wi.itemID == WeaponConfig::ITEMID_BlockEnergy)
	{
		Scaleform::GFx::Value var[3];
		var[0].SetString("Place barricade into quick slot and use it from there");
	...
	else
	{
		...
		if((wi.itemID == WeaponConfig::ITEMID_Bandages || wi.itemID == WeaponConfig::ITEMID_Bandages2 || wi.itemID == WeaponConfig::ITEMID_Antibiotics ||
			wi.itemID == WeaponConfig::ITEMID_Painkillers || wi.itemID == WeaponConfig::ITEMID_Medkit || wi.itemID == WeaponConfig::ITEMID_C01Vaccine || wi.itemID == WeaponConfig::ITEMID_C04Vaccine || fc && fc->category == storecat_Food)
			&& plr->Cooldown > 0)
		{
			Scaleform::GFx::Value var[3];
			var[0].SetString("$InfoMsg_CooldownActive");
			var[1].SetBoolean(true);
			var[2].SetString("");
			gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
			return;
		...
		if((wi.itemID == WeaponConfig::ITEMID_Bandages || wi.itemID == WeaponConfig::ITEMID_Bandages2 || wi.itemID == WeaponConfig::ITEMID_Antibiotics ||
			wi.itemID == WeaponConfig::ITEMID_Painkillers || wi.itemID == WeaponConfig::ITEMID_Medkit || wi.itemID == WeaponConfig::ITEMID_C01Vaccine || wi.itemID == WeaponConfig::ITEMID_C04Vaccine || fc && fc->category == storecat_Food) && plr->Cooldown <=0)
		{
			plr->currentslot = slotID;
			plr->Cooldown = r3dGetTime() + 10.0f;
		}
...
void HUDPause::eventChangeBackpack(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if (Size>SizeNewBack)
	{
		if (ItemsNumber>SizeNewBack)
		{
			Scaleform::GFx::Value var[2];
			var[0].SetStringW(gLangMngr.getString("$RemoveSomeItems"));
			var[1].SetBoolean(true);
			gfxMovie.Invoke("_root.api.showInfoMsg", var, 2);
		}
		else{
			Scaleform::GFx::Value var[2];
			var[0].SetStringW(gLangMngr.getString("$PlaceItems"));
			var[1].SetBoolean(true);
			gfxMovie.Invoke("_root.api.showInfoMsg", var, 2);
		}
		return;
	...
	for(int i=0; i<slot.BackpackSize; ++i)
	{
		...
		{
			if(slot.Items[i].quantity > 1) // antidupe
			{
				r3dMouse::Show();
				char Bcbksgs[64];
				sprintf(Bcbksgs, "Backpack dupe fix");
				Scaleform::GFx::Value var[3];
...
{
	if(gClientLogic().localPlayer_->inCombat)
	{
		Scaleform::GFx::Value var[3];
		char tmpMsg[64];
		sprintf(tmpMsg, "You are in combat....");
		var[0].SetString(tmpMsg);
		var[1].SetBoolean(false);
		var[2].SetString("COMBAT");
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
		return;
	}
...
#define MAKE_CALLBACK(FUNC) new r3dScaleformMovie::TGFxEICallback<HUDPause>(this, &HUDPause::FUNC)
	...
	gfxMovie.RegisterEventHandler("eventShowContextMenuCallback", MAKE_CALLBACK(eventShowContextMenuCallback)); // new menu
	gfxMovie.RegisterEventHandler("eventContextMenu_Action", MAKE_CALLBACK(eventContextMenu_Action)); // new menu
...
void HUDPause::Update()
{
	...
	if(DisconnectAt > 0)
	{
		...
		char tmpMsg[64];
		if(timeLeft > 0)
			sprintf(tmpMsg, "$HUD_DisconnectingIn %d $HUD_DisconnectingIn2", timeLeft);
		else
			sprintf(tmpMsg, "$HUD_DisconnectingSoon");
		var[0].SetString(tmpMsg);
		var[1].SetBoolean(false);
		var[2].SetString("$HUD_DisconnectingTitle");
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
...
///////////////////////////////
	{
		char tmpStr[256];
		sprintf(tmpStr, "$HUD_ServerNameTitle %s", gClientLogic().m_gameInfo.name);
...
void HUDPause::addMission()
{
	...
	if (plr->CurLoadout.Mission1 == 0)
		SetMissionInfo(1,false,false,"","Lets Play","PICK UP ITEM & KILL 5 ZOMBIE : 200XP","Missions/mission1.png",1);
	else if (plr->CurLoadout.Mission1 != 0 && plr->CurLoadout.Mission1 !=3)
		SetMissionInfo(1,false,true,"","Lets Play","PICK UP ITEM & KILL 5 ZOMBIE : 200XP","Missions/mission1_1.png",1);
	if (plr->CurLoadout.Mission2 == 0)
		SetMissionInfo(2,false,false,"","Super Zombie Hunter","KILL 5 SUPER ZOMBIE : $5000","Missions/mission2.png",2);
	else if (plr->CurLoadout.Mission2 != 0 && plr->CurLoadout.Mission2 !=3)
		SetMissionInfo(2,false,true,"","Super Zombie Hunter","KILL 5 SUPER ZOMBIE : $5000","Missions/mission2.png",2);
}
...
if (gClientLogic().localPlayer_)
{
	...
	gfxMovie.Invoke("_root.api.showInventory", 0);
	gfxMovie.Invoke("_root.api.showMap", 0);
	gfxMovie.Invoke("_root.api.showMissions", 1);
	gfxMovie.Invoke("_root.api.showOptions", 0);
}

bool HUDPause::isActiveMissions()
{
	return isMissionsOpen;
}
...
if (gClientLogic().localPlayer_)
{
	...
	for(int i=0; i<ClientGameLogic::MAX_NUM_PLAYERS; ++i)
	{
		...
		if(plr && plr != LocalPlr && plr->ClanID == LocalPlr->ClanID && plr->ClanID != 0)
		{
			...
			for (int u=0;u<10;u++)
			{
				if (strcmp(gClientLogic().localPlayer_->PlayersNames[u],gClientLogic().localPlayer_->CurLoadout.Gamertag) != 0 && strcmp(gClientLogic().localPlayer_->PlayersNames[u],"") != 0 && strcmp(gClientLogic().localPlayer_->PlayersNames[u],"") != 0) // Position of Players
				{
					//r3dOutToLog("Player %s Posicion X:%f posicion Y:%f\n",gClientLogic().localPlayer_->PlayersNames[u],gClientLogic().localPlayer_->PositionGroups[u].x,gClientLogic().localPlayer_->PositionGroups[u].y);
					r3dPoint2D pos = getMinimapPos(gClientLogic().localPlayer_->PositionGroups[u]);
					Scaleform::GFx::Value var[2];
					var[0].SetNumber(pos.x);
					var[1].SetNumber(pos.y);
