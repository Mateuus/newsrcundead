...
int externalcam; // Server Vehicles
	...
	if(!win::bSuspended && !hudMain->isChatInputActive() && !hudMain->isPlayersListVisible())
	{
		...
		if(showHudPause && !hudAttm->isActive() && !hudGeneralStore->isActive() && !hudVault->isActive() && !hudSafeLock->isActive() /*&& !hudRepair->isActive()*/)
		{
			if(!hudPause->isActive())
			{
				...
				else if(Keyboard->WasPressed(kbsH))
					hudPause->showMissions();
			}
			else {
				if (!hudPause->isActiveMissions() || Keyboard->WasPressed(kbsEsc))
				{
						hudPause->Deactivate();
...
if(hudMain && !hudMain->isChatInputActive())
			...
			if(showPlayerList)
			{
				if (!hudMain->isPlayersListVisible())
				{
					...
					for(int i=0; i<R3D_ARRAYSIZE(CGL.playerNames); i++)
					{
						...
						{
							char plrUserName[64]; gClientLogic().localPlayer_->GetUserNameA(plrUserName);
							bool isme=false;
...

		{
			...
			if(!pl->bDead && (r3dGetTime() - pl->TimeOfLastRespawn)>300.0f && !hudAttm->isActive() && !hudPause->isActive() 
			&& !hudActionUI->isActive() && !hudGeneralStore->isActive() && !hudVault->isActive())
			{
...
//#ifndef FINAL_BUILD //UndeadBrasil Spectator
    if (gUserProfile.ProfileData.isDevAccount){ // UndeadBrasil Para nenhum player poder usar o Specator somente Devs
	...
	if(d_video_spectator_mode->GetBool() || d_observer_mode->GetBool())
	{
      ...
      //extern float __EditorWalkSpeed;
        float __EditorWalkSpeed = 5.0f; //UndeadBrasil Spectator
