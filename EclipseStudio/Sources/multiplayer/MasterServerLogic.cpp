...
void MasterServerLogic::SendRentGame(const char* name,const char* pwd , int slot , int mapid , int period, int customerid, int Region, DWORD SesionID, bool CrosshairEnable, bool enableSnipers)
{
	if(!net_ || !net_->IsConnected())
		r3dError("masterserver isn't connected\n");
	if(badClientVersion_) 
	...
	n.period = period;
	n.SesionID = SesionID;
	n.EnableCrosshair = CrosshairEnable;
	n.enableSnipers = enableSnipers;
	net_->SendToHost(&n, sizeof(n));
}
...
void MasterServerLogic::SendRentGameChangePwd(int id , const char* pwd, int customid, int SesionID, bool EnableCrosshair, bool enableSnipers)
{
	if(!net_ || !net_->IsConnected())
		r3dError("masterserver isn't connected\n");
	if(badClientVersion_) 
	...
	n.customid = customid;
	n.SesionID = SesionID;
	n.EnableCrosshair = EnableCrosshair;
	n.enableSnipers = enableSnipers;
	net_->SendToHost(&n, sizeof(n));
}
