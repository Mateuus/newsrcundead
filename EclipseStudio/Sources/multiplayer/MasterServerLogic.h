...
virtual	void		OnNetData(DWORD peerId, const r3dNetPacketHeader* packetData, int packetSize);
  ...
  public:
	...
	void         SendRentGame(const char* name,const char* pwd , int slot , int mapid , int period, int customerid, int Region, DWORD SesionID, bool CrosshairEnable, bool enableSnipers);
	void         SendRentReq();
	void SendRentGameChangePwd(int id , const char* pwd, int customid, int SesionID, bool EnableCrosshair, bool enableSnipers);
