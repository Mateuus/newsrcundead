...
void CMasterGameServer::UpdateRentGames()
{
		...
		++it)
	{
		...
		for(int i=0; i<super->maxGames_; i++) 
		{
			...
			if (strcmp(game->info_.ginfo.PasswordGame,"") != 0)
			{
				__int64 secs1 = _time64(&secs1);
				if (secs1 > game->info_.ginfo.expirein)
					game->info_.ginfo.DataExpire = 0;
				else
					game->info_.ginfo.DataExpire = (((game->info_.ginfo.expirein-(int)secs1) /60) /60);
				if (secs1 > game->info_.ginfo.expirein) // expired server
				{
					gServerConfig->expired(game->info_.ginfo.gameServerId);
					r3dOutToLog("RentServer %d expired. send shut down..\n",game->info_.ginfo.gameServerId);
					strcpy(game->info_.ginfo.PasswordGame,"");
...
{
	r3dOutToLog("--------- SHUTDOWN on 60 secs, requested to serverid %d\n",gameServerId);
	SBPKT_M2G_ShutdownNote_s n;
	n.reason   = 0;
	n.timeLeft = 60;
	for(CMasterGameServer::TSupersList::const_iterator it = gMasterGameServer.supers_.begin();
