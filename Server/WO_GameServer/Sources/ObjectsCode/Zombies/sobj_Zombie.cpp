...
{
    if(plr->loadout_->Alive == 0 || (plr->loadout_->GameFlags & wiCharDataFull::GAMEFLAG_SpawnProtection) || plr->profile_.ProfileData.isGod)
        return false;

	//If repellent is used then return not detectable by zombies
	if(plr->loadout_->Repellent==1)
		return false;
		...
		case EZombieStates::ZState_Attack:
		{
			...
			if(attackTimer >= _zai_AttackTimer)
			{
				...
				else
				{
					...
					if(trg->loadout_->Health <= 0 || trg->loadout_->Health != trg->loadout_->Health)
					{
...
bool obj_Zombie::ApplyDamage(GameObject* fromObj, float damage, int bodyPart, STORE_CATEGORIES damageSource)
{
	...
	if(HeroItemID != 20204) // it's normal zombie
	{
		...
		{
			dmg = ((damageSource != storecat_MELEE && damageSource != storecat_punch)) ? 1000 : dmg; // everything except for melee: one shot in head = kill  // you can kill zombies with just 1 hit if it's weapon (not melee and not punch)
			ZombieHealth -= dmg; //add damage
		}
	}
	else
		ZombieHealth = (bodyPart == 1) ? (ZombieHealth - (dmg * 15 / 100)) : ZombieHealth; // it's superzombie, only hitting head will lower super zombie's health
