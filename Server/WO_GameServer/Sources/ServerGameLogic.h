...
virtual	void		OnNetData(DWORD peerId, const r3dNetPacketHeader* packetData, int packetSize);
  ...
  public:
	...
	int		ProcessWorldEvent(GameObject* fromObj, DWORD eventId, DWORD peerId, const void* packetData, int packetSize);
	 ...
	 DEFINE_PACKET_FUNC(PKT_S2C_UpdateSlotsCraft);
	 DEFINE_PACKET_FUNC(PKT_S2C_RentAdminCommands);
	 ...
	 DEFINE_PACKET_FUNC(PKT_C2S_FireToClaymore);
	...
	int		  Cmd_TeleportAllToMe(obj_ServerPlayer* plr, const char* cmd);
	int		  Cmd_Kick(obj_ServerPlayer* plr, const char* cmd);
	int		  Cmd_crtveh(obj_ServerPlayer* plr, const char* cmd);
